...
# Notes: I you want to edit the Ins/Cmd or 🖋 / 🛠
# please visit /etc/inputrc
...
# Display subdirectories in a graphical manner
alias subdirs='ls -R | grep ":$" | sed -e '\''s/:$//'\'' -e '\''s/[^-][^\/]*\//--/g'\'' -e '\''s/^/ /'\'' -e '\''s/-/|/'\'''
...
# Print ls after cd
function cd {
  # clear && builtin cd "$@" && ls * && echo ''
  clear && builtin cd "$@" && ls -a && echo ''
}

# TEMPORARY:
# Adding pip to PATH as it is needed by manual 
